...
class Ui_mainWindow(object):
    ...
    def retranslateUi(self, mainWindow):
        ...
        mainWindow.setWindowTitle(_translate("mainWindow", "SURFr"))
        ...
        self.processButton.setText(_translate("mainWindow", "Let\'s SURF!"))
    ...
    def preprocessButtonClicked(self):
        ...
        try:
            Procedure.separate_seqs(self.dbFile, self.seqFile, self.tempSeparatedFile, 18)
        except:
            traceback.print_exc()
    ...
    def processButtonClicked(self):
        ...
        try:
            ...
            get_peaks(self.results, self.orig_strings, self.exportedResultsSeq, self.exportedResultsDB)
        except:
            traceback.print_exc()
    ...
    def plotButtonClicked(self):
        ...
        if selection == '':
            try:
                ...
                peaks, _ = find_peaks(vector, width=16)
                prom = peak_prominences(vector, peaks)
                rh = 0.7
                widths, _, _, _ = peak_widths(vector, peaks, rh, prom)
                self.uidLabel.setText(uid)
                self.sc.axes.plot(vector, label='Expression')
                self.sc.axes.plot(peaks, vector[peaks], 'ko', label='Peaks')
                self.sc.axes.legend()
                if peaks.size != 0:
                    for i, peak in enumerate(peaks):
                        if widths[i] > 16:
                            self.sc.axes.vlines([peak - widths[i] / 2, peak + widths[i] / 2], ymin=0, ymax=vector[peak], color='g', linestyle='dashed')
                            self.sc.axes.fill_betweenx([0, vector[peak]], peak - widths[i] / 2, peak + widths[i] / 2, color='g', alpha=0.2)
            ...
            try:
                ...
                rh = 0.35
                widths, _, _, _ = peak_widths(vector, peaks, rh, prom)
                self.uidLabel.setText(str(selection))
                self.sc.axes.plot(vector)
                self.sc.axes.plot(vector, label='Expression')
                self.sc.axes.plot(peaks, vector[peaks], 'ko', label='Peaks')
                self.sc.axes.legend()
                if peaks.size != 0:
